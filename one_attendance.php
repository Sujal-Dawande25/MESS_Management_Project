...
<body>
    <div class="container">
        <center><h1 class="details-heading">One Time Attendance</h1></center>
...
if (!$conn) {
    ...
    if (isset($_POST['subscription_id']) && isset($_POST['attendance'])) {
        ...
        if ($attendance === 'Absent') {
            ...
            $query = "SELECT o_total_no_of_absentees FROM one WHERE o_subscription_id = $subscription_id";
            $result = pg_query($conn, $query);
            $row = pg_fetch_assoc($result);
            $currentAbsentees = (int)$row['o_total_no_of_absentees'];
            ...
            if ($currentAbsentees < 4) {
                ...
                $update_query = "UPDATE one SET o_total_no_of_absentees = $newAbsentees, attendance = 'Absent' WHERE o_subscription_id = $subscription_id";
                ...
                if ($result) {
                    ...
                    $endDateQuery = "SELECT end_date FROM one WHERE o_subscription_id = $subscription_id";
                    ...
                    $updateEndDateQuery = "UPDATE one SET end_date = '$newEndDate' WHERE o_subscription_id = $subscription_id";
    ...
    // Query to fetch data from the 'one' table
    $query = "SELECT o_subscription_id, o_username, o_usertype, attendance ,o_total_no_of_absentees FROM one";
    $result = pg_query($conn, $query);
    ...
    if (pg_num_rows($result) > 0) {
        ...
        while ($row = pg_fetch_assoc($result)) {
            echo "<tr id='row_" . $row['o_subscription_id'] . "'>"; // Assign a unique ID to each row
            echo "<td>" . $row['o_subscription_id'] . "</td>";
            echo "<td>" . $row['o_username'] . "</td>";
            echo "<td>" . $row['o_usertype'] . "</td>";
            echo "<td id='status_" . $row['o_subscription_id'] . "'>" . $row['attendance'] . "</td>";
            echo "<td id='actions_" . $row['o_subscription_id'] . "'>" .
                "<form method='post'>" . // Form for submitting attendance updates
                "<input type='hidden' name='subscription_id' value='" . $row['o_subscription_id'] . "'>" .
                "<input type='hidden' name='table' value='one'>" . // Hidden field for table name
                ...
                "</form>" .
                "</td>";
            echo "<td id='total_absentees_" . $row['o_subscription_id'] . "'>" . $row['o_total_no_of_absentees'] . "</td>"; 
            echo "</tr>";
        }
    } else {
